...
if __name__=="__main__":
	...
	# MOD: create directory for image frames and file for state vectors
	base_frame_dir = "frames"
	if not os.path.exists(base_frame_dir):
		os.makedirs(base_frame_dir)

	frame_dir = "frames/" + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d-%H-%M-%S')
	if not os.path.exists(frame_dir):
		os.makedirs(frame_dir)

	resultFile = open("./state_vectors_" + str(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d-%H-%M-%S')) + ".csv",'w')
	wr = csv.writer(resultFile, delimiter=',')
	wr.writerow(['step', 'pos_x', 'pos_y', 'vel_x', 'vel_y', 'ship_lander_angle', 'ship_lander_angular_vel', 'leg_1_ground_contact', 'leg_2_ground_contact', 'action'])
	...
	for i in range(0, 1000):
		...
		while True:
		    ...
		    # MOD: write the state vector to a csv file
		    wr.writerow(list([steps] + list(s) + [a]))
		    if(steps % 100 == 0):
		        resultFile.flush()

		    # MOD: Get raw image arrays
		    raw_images = env.render(mode='rgb_array')
		    # MOD: Write them to frames directory
		    im = Image.fromarray(raw_images)
		    im.save(frame_dir+"/frame_" + str(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d-%H-%M-%S')) + "_" + str(steps) + "_" + str(a) + ".jpeg")
